...
import { getActiveSessionMinutes, getPendingInactiveMinutes } from "./session-runtime";
...
export function collectSignal(): UserSignal {
  ...
  const inactiveSessionMinutes = getPendingInactiveMinutes();
  const distractingSessionMinutes =
    overrideMinutes !== undefined
      ? overrideMinutes
      : Math.max(20, activeSessionMinutes, inactiveSessionMinutes);
