...
1. **Onboarding** (`app/index.tsx`): extended first-install flow (`ExtendedOnboardingFlow`). Scrollable copy, Dynamic Type up to Accessibility XXL. **How it works:** headline + four steps; **Tap the circles — it's the button here** appears in the fixed circles block. Footer: **About** only. Shown once per install (`extended_onboarding_completed`).
2. **Trigger** (`app/trigger.tsx`): same circles slot; main prompt (**One action for you**) and footer **Show my paths** / **Мої шляхи** fade in **together**; short **tap to continue** hint appears under the circles (later). Paths → `app/paths.tsx`.
3. **Action** (`app/action.tsx`): one micro-intervention (feet / find 3 / triangle breath / relax jaw / drop shoulders / notice 3 sounds / press palms together). Main line pinned at shared Y; bullets / phases flow **below** main while the hint stays in the fixed circles block. **Find 3 things** shows three cues from **7 rotating sets** (`find-three-variants.ts`); same set never repeats back-to-back. Until all three bullets are visible, tapping the circles reveals the next bullet instead of completing the action. **Triangle breath**: soft haptic at inhale start and light haptic at exhale start (`startTriangleBreathHapticLoop`). The **tap to continue** hint appears **last** (after all bullets or after 3 breath cycles). **Circles use the same visual everywhere** (`spiral-visual.ts` + `SpiralRings`). Action → return uses **`router.replace`** (no stack fade — `animation: "none"` on flow screens).
4. **Return** (`app/return.tsx`): **You are here** (pinned main, fade-in) → explanation (fade after main); **tap to continue** appears under the circles after the explanation. Optional **Save this for me** in the **footer** (becomes **Saved**, not clickable; hidden on later visits if already saved). Tap circles → trigger.
...
| Circles hint | **Onboarding**: `Tap the circles — it's the button here` in the fixed circles block after the onboarding steps. **Trigger / action / return**: `tap to continue` under the circles, after all screen copy on that screen (`getFlowSpiralHintDelayMs`). Shown only for the first **3 completed cycles**, then hidden. **Triangle breath**: hint after **3 breath cycles**. |
