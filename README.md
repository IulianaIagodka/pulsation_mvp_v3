...
4. **Return** (`app/return.tsx`): “You are here” + short explanation (centered); optional **Save this for me** in the **footer** (becomes **Saved**, not clickable; hidden on later visits if already saved). Short spiral hint under the rings; tap spiral → trigger.
...
| Spiral hint (“tap the spiral”) | **Onboarding**: inline after intro (last line). **Other flow screens**: short label under the spiral (`SpiralUnderHint`), after all screen copy (`getFlowSpiralHintDelayMs`). Shown only for the first **3 completed cycles**, then hidden. **Triangle breath**: hint after **3 breath cycles**. |
