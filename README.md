...
1. **Onboarding** (`app/index.tsx`): **first install** — full flow (`ExtendedOnboardingFlow`): **Pulsation exists…** stays visible; **How it works:** + four steps appear below — **When scrolling becomes automatic** → **One quiet invitation, one small action** → **Tap the circles to continue** → **Then it stays in the background** (auto one-by-one, or first circles tap reveals all together); second tap → trigger; footer **About** only; once per install (`extended_onboarding_completed`). **Later cold starts** — short headline + **About**; tap circles → trigger. **Resume from background** — skip onboarding, open **One action for you** directly.
