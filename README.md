...
When Pulsation is in the background, it schedules only the next local **one-action invitation** at a **dynamic interval** near 20 minutes (kept in a 10-30 minute window and adapted by recent completions, ignores, and absence — see `docs/adaptive-scheduling.md`):
...
Timing adapts gently — it is not a fixed 20-minute reminder — and returning to the app clears the pending invitation.
