...
Перший екран (**розширений онбординг**): заголовок + «How it works» / «Як це працює» + 4 кроки + «Tap the spiral» — у режимі знімка весь текст у **одному скролі**, компактніше, видно одразу (⌘S знімає те, що на екрані — прокрути вниз, якщо на малому симуляторі не вміщається).
